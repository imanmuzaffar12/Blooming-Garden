...
function ReportFormLayoutByRepoTyp(disableBothControls) {
    ...
    if (pgSection > 0) {
        ...
        else if (pgSection == 4 || pgSection == 7 || pgSection == 8) {// || pgSection == 6) {
            $('#dvSectors').hide();
            $('#dvCompany').show();
...
function onChangeFiscalPeriodTypes(selVal) {
    ...
    if ((pgSection == 4 || pgSection == 5) && (fpType == 3 || fpType == 5)) {
        alert('Please select Month or Year');
        $('#FiscalPeriodTypes').val(selVal);
    }
    else if ((pgSection == 8 || pgSection == 9) && (fpType == 2 || fpType == 3 || fpType == 5)) {
...
function AddFldRow() {
    ...
    if ($('#frmFldSelection').valid()) {
        ...
        var liItm = ListItemText(id,repoFldId, attOrFFText, 0);
...
function alertMessage(message) {
    ...
    $("#errorMessage").css("display", "block").fadeToggle(5000);
}
function ShowMessage(message) {
    $("#addEditForm").scrollTop();
