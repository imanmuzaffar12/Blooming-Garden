...
namespace PetaPoco
{
    ...
    public partial class Database : IDisposable
    {
            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, sql, args))
                    {
                        ...
                        foreach (System.Data.SqlClient.SqlParameter sqlParam in cmd.Parameters)
                        {
                            actualQuery = actualQuery.Replace(sqlParam.ParameterName, sqlParam.Value.ToString());
                        }
        ...
        /// as the primary key the id of the new record is assigned to it.  Either way,
        /// the new id is returned.</remarks>
        public object Insert(string tableName, string primaryKeyName, bool autoIncrement, bool disableOutPut, object poco)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        string outputClause = String.Empty;
                        if (autoIncrement && !disableOutPut)
                        {
                            outputClause = _dbType.GetInsertOutputClause(primaryKeyName);
                        }
                        ...
                        // Assign the ID back to the primary key property
                        if (primaryKeyName != null && !disableOutPut)
                        {
                            PocoColumn pc;
                            if (pd.Columns.TryGetValue(primaryKeyName, out pc))
                            {
	...
	*/
    ...
    public class Sql
    {
        ...
        /// <summary>
        /// This method apply paging for SQL server 2012
        /// </summary>
        /// <param name="pageNo"></param>
        /// <param name="pageSize"></param>
        /// <returns></returns>
        public Sql Paginate(int pageNo, int pageSize)
        {
            var sql_ = Append(new Sql(string.Format("OFFSET {0} ROWS", (pageNo - 1) * pageSize)));
            sql_.Append(new Sql(string.Format("FETCH NEXT {0} ROWS ONLY", pageSize)));
            return sql_;
        }
        /// <summary>
                    ...
                    else
                    {
                        ...
                        foreach (var o in args_src)
                        {
                            ...
                            else
                            {
                                try
                                {
                                    var items = ((IDictionary<string, object>)o);
